...
class AlienFleet:
    alien_exploding_images = [pg.image.load(f'images/explosion{n}.png') for n in range(10)]
    ...
    alien_images = [[pg.transform.rotozoom(pg.image.load(f'images/alien___{m}{n}.png'), 0, 1) for n in range(2)] for m in range(3)]
    ufo_imgs = [pg.transform.rotozoom(pg.image.load(f'images/ufo___{n}.png'), 0, 1) for n in range(2)]
    ...
    def create_alien(self, row, col):
        x = self.alien_w * (1 * col + 1)
        y = self.alien_h * (1 * row + 1)
...
class Alien(Sprite):
    def __init__(self, game, image_list, alien_index, sound, start_index=0, ul=(0, 100), v=Vector(1, 0),
        ...
        self.image = pg.image.load('images/ufoxxx.bmp')
        self.screen_rect = self.screen.get_rect()
